...
Alternatively, install `Qt Creator`_ and open the `bitgoldcoin-qt.pro` file.

An executable named `bitgoldcoin-qt` will be built.
...
To use UPnP for port forwarding behind a NAT router (recommended, as more connections overall allow for a faster and more stable bitgoldcoin experience), pass the following argument to qmake:
...
installed causes bitgoldcoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
